...
#         process = subprocess.Popen([sys.executable,"plzchaljao.py",st.session_state['user_name']], creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
...
if not st.session_state['logged_in']:
    ...
    if st.button("Start Facial Emotion Recognition"):
        ...
        # Check the platform and set flags accordingly
        if platform.system() == 'Windows':
            creationflags = subprocess.CREATE_NEW_PROCESS_GROUP
        else:
            creationflags = 0  # No special flag for Unix-based systems
        
        process = subprocess.Popen(
            ["python", "plzchaljao.py", st.session_state['user_name']],
